...
function AuthForm() {
  ...
  function validateBirth(birth) {
    ...
    if (
      ...
      return false;
    const now = new Date();
    if (year < 1900 || date > now) return false;
    const minBirth = new Date(
      now.getFullYear() - 18,
      now.getMonth(),
      now.getDate()
    );
      if (date > minBirth) return "underage";
      return true;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      if (!validateName(name))
        return setError(
          "이름은 한글 2~10자 또는 영문 2~20자(이름/성)만 입력 가능합니다."
        );
      if (!validateBirth(birth))
          return setError(
            "생년월일을 YYYY-MM-DD 형식의 실제 날짜로 입력해 주세요."
          );
      if(validateBirth(birth)=="underage")
          return setError("만 18세 미만은 가입할 수 없습니다.");
      if (!validateAddress(address))
        return setError("주소는 5자 이상 입력해 주세요.");
      if (!validatePassword(password))
        return setError(
          "비밀번호는 8자 이상, 영문/숫자/특수문자를 모두 포함해야 합니다."
        );
      try {
        ...
        if (err.code === "auth/email-already-in-use")
          ...
          setError(
            "회원정보 저장 중 오류가 발생했습니다. 네트워크 상태를 확인하거나 잠시 후 다시 시도해 주세요."
          );
        else setError("오류가 발생했습니다. 다시 시도해 주세요.");
      }
    }
    ...
    <div className="auth-box" data-testid="auth-form">
      <form onSubmit={handleSubmit} className="auth-form">
        ...
        <button
          ...
          disabled={
            ...
            !password ||
            (!isLogin &&
              (!name || !birth.match(/^\d{4}-\d{2}-\d{2}$/) || !address))
          }
        >
          {isLogin ? "로그인" : "회원가입"}
        </button>
      ...
      {showReset && (
        <form className="reset-form" onSubmit={handlePasswordReset}>
          ...
          />
          <button type="submit" className="main-btn" style={{ marginTop: 6 }}>
            재설정 메일 전송
          </button>
          <span className="switch-link" onClick={() => setShowReset(false)}>
            닫기
          </span>
        </form>
      )}
